...
(cl-defmethod gp--pull-request-merge-strategies ((_ git-platform-github) full-name id)
  (github-pull-request-merge-strategies full-name id))
(cl-defmethod gp--pull-request-mergeability ((_ git-platform-github) full-name id)
  (github-pull-request-mergeability full-name id))
(cl-defmethod gp--pull-request-divergence ((_ git-platform-github) full-name base head)
  (github-pull-request-divergence full-name base head))
...
(cl-defmethod gp--labels-supported-p ((_ git-platform-github)) t)
(cl-defmethod gp--inline-target-problem ((_ git-platform-github) full-name id path line)
  (github-inline-target-problem full-name id path line))
(cl-defmethod gp--reactions-supported-p ((_ git-platform-github)) t)
(cl-defmethod gp--reaction-choices ((_ git-platform-github)) github-reaction-contents)
(cl-defmethod gp--comment-reactions ((_ git-platform-github) full-name comment)
  (github-comment-reactions full-name comment))
(cl-defmethod gp--set-comment-reaction ((_ git-platform-github) full-name comment content on)
  (if on
      (and (github-add-comment-reaction full-name comment content) t)
    (github-remove-comment-reaction full-name comment content)))
(cl-defmethod gp--pr-labels ((_ git-platform-github) pr)
  (github-pr-labels pr))
(cl-defmethod gp--repo-labels ((_ git-platform-github) full-name)
  (github-repo-labels full-name))
(cl-defmethod gp--set-pull-request-labels ((_ git-platform-github) full-name id labels)
  (github-set-pull-request-labels full-name id labels))
...
(cl-defmethod gp--pipelines-for-branch-async ((_ git-platform-github) full-name branch max-items commit callback)
  (github-pipelines-for-branch-async full-name branch max-items commit callback))
...
(cl-defmethod gp--pipeline-steps-async ((_ git-platform-github) full-name pipeline-uuid callback)
  (github-pipeline-steps-async full-name pipeline-uuid callback))
...
(cl-defmethod gp--pr-web-url ((_ git-platform-github) pr)
  ;; GitHub is flat: `html_url', no `links' object at all
  (or (alist-get 'html_url pr) (let-alist pr .links.html.href)))
(cl-defmethod gp--comment-web-url ((_ git-platform-github) comment)
  ;; comments are reshaped into the shared shape, so they DO carry links
  (or (let-alist comment .links.html.href) (alist-get 'html_url comment)))
...
;; GitHub has no decline-reason field; a closed PR's explanation is a comment.
(cl-defmethod gp--pr-closed-reason ((_ git-platform-github) _pr) nil)
(cl-defmethod gp--pr-merged-at ((_ git-platform-github) pr)
  (or (alist-get 'merged_at pr) (alist-get 'updated_at pr)))
(cl-defmethod gp--pr-merge-commit ((_ git-platform-github) pr)
  (alist-get 'merge_commit_sha pr))
...
(cl-defmethod gp--commit-message-async ((_ git-platform-github) full-name hash callback)
  (github-commit-message-async full-name hash callback))
